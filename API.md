...
-   `config` **[Object][2]** configuration object
    -   `config.id` **[string][3]** Connector ID - required
    -   `config.secret` **[string][3]** Connector Secret - required
    -   `config.organization` **[string][3]** Hull organization - required
    -   `config.requestId` **[string][3]?** additional parameter which will be added to logs context, it can be HTTP request unique id when you init HullClient and you want to group log lines by the request (it can be a job id etc.)
    -   `config.connectorName` **[string][3]?** additional parameter which will be added to logs context, it's used to track connector name in logs
    -   `config.firehoseUrl` **[string][3]?** The url track/traits calls should be sent - deprecated option, will be removed in next version
    -   `config.protocol` **[string][3]** protocol which will be appended to organization url, override for testing only (optional, default `https`)
    -   `config.prefix` **[string][3]** prefix of Hull REST API (optional, default `/api/v1`)
...
-   `additionalClaims` **[Object][2]**  (optional, default `{}`)
    ...
    -   `additionalClaims.active` **[string][3]** marks the user as _active_ meaning a reduced latency at the expense of scalability. Don't use for high volume updates (optional, default `false`)
...
-   `context` **[Object][2]**  (optional, default `{}`)
    -   `context.source` **[string][3]?** optional source prefix, if applied all traits will be prefixed with this string (and `/` character)
    -   `context.sync` **[string][3]** make the operation synchronous - deprecated option, will be removed in next version (optional, default `false`)
...
-   `context` **[Object][2]** The `context` object lets you define event meta-data. Everything is optional (optional, default `{}`)
    -   `context.source` **[string][3]?** Defines a namespace, such as `zendesk`, `mailchimp`, `stripe`
    -   `context.type` **[string][3]?** Define a event type, such as `mail`, `ticket`, `payment`
    -   `context.created_at` **[string][3]?** Define an event date. defaults to `now()`
    -   `context.event_id` **[string][3]?** Define a way to de-duplicate events. If you pass events with the same unique `event_id`, they will overwrite the previous one.
    -   `context.ip` **[string][3]?** Define the Event's IP. Set to `null` if you're storing a server call, otherwise, geoIP will locate this event.
    -   `context.referer` **[string][3]?** Define the Referer. `null` for server calls.
